//
//  JBWLAppointCourseManageController.m
//  JBApp
//
...
#import "JBWLAppointCourseController.h"

#import "XFQNavigationView.h"
#import "WQScrollTitileView.h"

@interface JBWLAppointCourseManageController ()
@property (nonatomic,strong)UIScrollView *bgScrollview;
@property (nonatomic, strong) NSMutableArray <JBWLAppointCourseController *> *listVCArray;
...
@implementation JBWLAppointCourseManageController
...
- (void)_setNav{
    ...
    XFQNavigationView  *nav =  [[XFQNavigationView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, WQ_StatusBarAndNavigationBarHeight) leftImageName:@"JbAPP_Back" leftTitle:nil title:@"我约的课" target:self action:@selector(gotoBack) showBottomLine:YES];
...
- (void)setTitleCurrentSelectedIndex:(NSInteger)titleCurrentSelectedIndex{
    ...
    JBWLAppointCourseController * currentVC = self.listVCArray[titleCurrentSelectedIndex];
...
- (void)_createUI{
    ...
    for (int i = 0; i < self.titlesArray.count; i++) {
        JBWLAppointCourseController * teachOrderController = [[JBWLAppointCourseController alloc] initWithType:i];
        teachOrderController.managerVC = self;
...
- (NSMutableArray <JBWLAppointCourseController *> *)listVCArray{
    if (!_listVCArray) {
